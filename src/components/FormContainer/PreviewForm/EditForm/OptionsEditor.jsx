...
const OptionsEditor = ({ displayId, options, setFormData, handleToggle }) => {
  ...
  return (
    <div className="mt-1 flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
      <div onClick={() => handleToggle("options-editor")} className="text-base cursor-pointer flex items-center justify-between font-semibold text-slate-800">
        <span>Options Editor</span>
        {displayId === "options-editor" ? <GoChevronUp className="w-7 h-7" /> : <GoChevronDown className="w-7 h-7" />}
      </div>

      {displayId === "options-editor" &&
        <>
          ...
          <div className="mt-1 grid grid-cols-1 gap-3 md:grid-cols-[3fr_3fr_0.8fr]">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 disabled:cursor-not-allowed disabled:bg-slate-100"
              type="text"
            />
            ...
            <button onClick={handleClick} className="cursor-pointer rounded bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-700"> {editIndex !== null ? "Edit" : "Add"}</button>
